...
function Monetario(valor) {
    return ("valor").maskMoney(preco_custo, {
        prefix: 'R$ ',
        allowNegative: true,
        thousands: '.',
        decimal: ',',
...
function ValidarCNPJ(ObjCnpj) {
    ...
    if (((dig1 * 10) + dig2) != digito) {
        ...
        $('input#cnpj').parent().find('span').remove();

        $('input#cnpj').addClass('has-error');
        $('input#cnpj').after('<span style="color:red">CNPJ inválido</span>');
    } else {
        $('input#cnpj').removeClass('has-error');
        $('input#cnpj').parent().find('span').remove();
